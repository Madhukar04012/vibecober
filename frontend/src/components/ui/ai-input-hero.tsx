...
export function HeroWave({
  ...
  useEffect(() => {
    ...
    // Pointer tracking
    function setupPointerTracking(rectGetter: () => DOMRect) {
    ...
    function initWaveThree() {
      ...
      const waveCamera = new THREE.OrthographicCamera(
        -cameraWidth / 2,
        cameraWidth / 2,
        cameraHeight / 2,
        -cameraHeight / 2,
      ...
      const waveComposer = new EffectComposer(waveRenderer);
      ...
      const wave1Timeline = buildKeyframeTweens(wave1, waveKeyframes1);
      const wave2Timeline = buildKeyframeTweens(wave2, waveKeyframes2);

      const mainTimeline = gsap.timeline({ repeat: -1 });
      mainTimeline.add(wave1Timeline, 0);
      mainTimeline.add(wave2Timeline, 0);

      const ro = new ResizeObserver((entries) => {
        for (const e of entries) {
          if (e.target === waveContainer) {
            cameraWidth = e.contentRect.width;
            cameraHeight = e.contentRect.height;
            ...
            waveRenderer.setPixelRatio(EFFECT_PR);
            waveRenderer.setSize(cameraWidth, cameraHeight);
            (waveComposer as any).setSize(cameraWidth, cameraHeight);
            (waveBloomPass as any)?.setSize(cameraWidth, cameraHeight);
            (grainPass as any)?.setSize(cameraWidth, cameraHeight);
      ...
      return { waveComposer, waveCamera, ro };
    }

    // Init
    const { waveComposer } = initWaveThree();
    ...
    >
      ...
      >
        <div className="max-w-3xl w-full text-center" style={{ pointerEvents: "auto" }}>
          ...
          >
            <div className="relative w-full sm:w-[720px]">
              ...
              <button
                ...
                className={`absolute right-3 bottom-3 inline-flex items-center justify-center w-10 h-10 rounded-xl
                  bg-[#f0f2ff] text-black hover:bg-white transition
                  border border-black/10 shadow-[0_4px_20px_rgba(0,0,0,0.25)]
                  ${!prompt.trim() ? "opacity-40 cursor-not-allowed hover:bg-[#f0f2ff]" : ""}`}
              >
                <svg
                  viewBox="0 0 24 24"
                  fill="none"
          ...
          <p className="mt-2 text-xs text-white/40 text-center">Press Enter to generate</p>
        </div>
      </div>
