...
export function IssueCard({
  ...
  status?: 'optional' | 'connecting' | 'connected' | 'unavailable' | 'error';
  disabled?: boolean;
  onPrimary?: () => void;
  className?: string;
}) {
    ...
    <Card className={cn('overflow-hidden', className)}>
        ...
        <div className="flex items-center gap-2">
          {status ? (
            <StatusPill
              ...
              variant={status === 'connected' ? 'success' : status === 'error' ? 'error' : 'info'}
            />
          ) : null}
          <span className="text-[11px] text-slate-500">Integration</span>
        </div>
      </div>
      ...
      <div className="px-3 py-3">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          {secondaryActionLabel ? (
            <Button variant="ghost" size="sm" aria-label={secondaryActionLabel} className="h-8 px-2 text-slate-200">
