...
export function CommandLog({ message, isLatest = false }: CommandLogProps) {
    ...
    return (
        <div className="border-b border-white/5 py-2">
            <div className="flex items-start gap-2">
                {/* Minimal role indicator - only show for latest */}
                {isLatest && (
                    <div
                        className={cn(
                ...
                <div className="min-w-0 flex-1">
                    {/* Show author name only for latest message */}
                    {isLatest && (
                    ...
                    {message.kind === 'thinking' ? (
                        <div className="flex items-center gap-2">
                            ...
                            <div className="h-1 w-16 overflow-hidden rounded-full bg-white/5">
                                <div className="h-full w-1/2 animate-[shimmer_1s_infinite] bg-violet-500/30" />
                            </div>
                        </div>
                    ) : (
